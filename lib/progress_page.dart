      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: selected
                ? (isDark
                      ? kDarkAccent.withOpacity(0.45)
                      : const Color(0xFFB8E7E1))
            ...
            Opacity(
              ...
              child: Image.asset(
                ...
                errorBuilder: (context, error, stackTrace) {
                  return Icon(icon, color: accentColor, size: 58);
                },
              ),
            ),
            const SizedBox(height: 16),
    ...
    return FutureBuilder<List<ChartPoint>>(
      ...
      builder: (context, snapshot) {
        ...
        if (snapshot.connectionState == ConnectionState.waiting) {
          return Center(
            child: CircularProgressIndicator(
              color: isDark ? kDarkAccent : _mint,
            ),
          );
        }
...
class _BlueLightScatterChartState extends State<BlueLightScatterChart> {
  ...
  String _timeBucketLabel(TimeRange range) {
    switch (range) {
      case TimeRange.daily:
        return 'hour';
      case TimeRange.weekly:
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<List<ScatterChartPoint>>(
      ...
      builder: (context, snapshot) {
        ...
        if (data.isEmpty) {
          return const _EmptyChartState(
            sectionLabel: 'Blue Light',
            icon: Icons.remove_red_eye_outlined,
            title: 'No blue light data yet',
            message:
                'Blue light data will appear here once readings are available for the selected time range.',
            accentColor: _mint,
          );
        }
